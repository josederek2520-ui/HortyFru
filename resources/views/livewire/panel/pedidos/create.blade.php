    ...
    <form wire:submit="save" class="flex flex-col gap-6">
        <section aria-labelledby="order-general-information" class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
            ...
            <div class="grid gap-5 p-5 sm:p-6 lg:grid-cols-2">
                ...
                <label class="block">
                    <span class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Fecha requerida <span class="text-error-500">*</span></span>
                    <input type="date" wire:model="form.fecha_requerida_pedido" @if (! $editing) min="{{ today()->toDateString() }}" @endif class="h-12 w-full rounded-xl border border-gray-300 bg-transparent px-4 text-sm text-gray-800 shadow-theme-xs outline-none transition focus:border-brand-500 focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white">
        ...
        <section aria-labelledby="order-products" class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
            <div class="flex flex-col gap-4 border-b border-gray-200 px-5 py-4 dark:border-gray-800 sm:flex-row sm:items-center sm:justify-between sm:px-6">
                <div class="flex items-center gap-3">
                    ...
                    <div><h2 id="order-products" class="font-semibold text-gray-900 dark:text-white">Productos solicitados</h2><p class="text-sm text-gray-500 dark:text-gray-400">Agrega una fila por cada artículo y presentación solicitada.</p></div>
                </div>
            ...
            <div class="flex flex-col gap-4 p-5 sm:p-6">
                ...
                @foreach ($form->detalles as $index => $detalle)
                    ...
                    <article wire:key="create-order-detail-{{ $index }}" class="rounded-2xl border border-gray-200 bg-gray-50/60 p-4 dark:border-gray-800 dark:bg-white/[0.02] sm:p-5">
                        ...
                        <div class="mt-4 grid gap-4 lg:grid-cols-12">
                            <label class="block lg:col-span-4"><span class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Artículo <span class="text-error-500">*</span></span><select wire:model="form.detalles.{{ $index }}.articulo_id" wire:change="selectArticle({{ $index }}, $event.target.value)" class="h-12 w-full rounded-xl border border-gray-300 bg-white px-3 text-sm text-gray-800 outline-none transition focus:border-brand-500 focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white"><option value="">Selecciona un artículo</option>@foreach ($this->articleOptions as $articulo)<option value="{{ $articulo->id }}">{{ $articulo->nombre_articulo }}</option>@endforeach</select>@error("form.detalles.$index.articulo_id")<span class="mt-1.5 block text-xs text-error-500">{{ $message }}</span>@enderror</label>
                            <label class="block lg:col-span-3"><span class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Presentación <span class="text-error-500">*</span></span><select wire:model="form.detalles.{{ $index }}.presentacion_articulo_id" @disabled($articuloSeleccionado === null) class="h-12 w-full rounded-xl border border-gray-300 bg-white px-3 text-sm text-gray-800 outline-none transition focus:border-brand-500 focus:ring-3 focus:ring-brand-500/10 disabled:cursor-not-allowed disabled:opacity-60 dark:border-gray-700 dark:bg-gray-900 dark:text-white"><option value="">Selecciona</option>@foreach ($articuloSeleccionado?->presentaciones ?? [] as $presentacion)<option value="{{ $presentacion->id }}">{{ $presentacion->nombre_presentacion_articulo }}</option>@endforeach</select>@error("form.detalles.$index.presentacion_articulo_id")<span class="mt-1.5 block text-xs text-error-500">{{ $message }}</span>@enderror</label>
